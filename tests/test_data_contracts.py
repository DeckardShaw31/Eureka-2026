...
def test_bilateral_trade_zero_preservation(bilateral_df):
    """Kiểm tra nguyên tắc PPML: Không được biến số 0 thành NaN hoặc xóa số 0"""
    assert (bilateral_df['trade_usd'] >= 0).all()
    zero_trades = (bilateral_df['trade_usd'] == 0).sum()
    assert zero_trades > 0, "Bảng song phương phải bảo toàn các quan sát thương mại bằng 0 cho PPML!"
